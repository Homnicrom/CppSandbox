...
- **MemorySafetyDrills**: single-file memory-safety "drills" meant to be built with AddressSanitizer. The current file's content is "Dangling reference" (a reference into a `std::vector` invalidated by reallocation) in `main.cpp`. Expect this file's content to be swapped out for different drills over time; check the comment header at the top of the file, not the filename, to know which drill is currently in place.

## Build

CMake, driven by `CMakePresets.json` at the repo root. Two configure presets, `windows` (Visual Studio 17 2022, x64) and `linux` (Ninja Multi-Config, `g++-14`), each with a matching `<name>-debug` build and test preset:

```bash
cmake --preset linux            # or: --preset windows
cmake --build --preset linux-debug
ctest --preset linux-debug
```

Both presets carry a `condition` on `${hostSystemName}`, so the wrong one errors out instead of half-configuring. `Ninja Multi-Config` (rather than plain Ninja or Unix Makefiles) is chosen deliberately on Linux: it is a multi-config generator like the VS one, so `--config Debug` and the `build/bin/<Target>/Debug/` layout behave identically on both platforms and there is a single set of commands to document. `CMakeUserPresets.json` is git-ignored for personal variants. Plain `cmake -B build -G ...` still works; presets are only a named argument list and change nothing about the targets.

The root `CMakeLists.txt` also hard-rejects GCC older than 14 with a `FATAL_ERROR`. `target_compile_features(... cxx_std_23)` cannot catch that case: GCC 13 accepts `-std=c++23`, so configure succeeds and the build fails later on a missing `<print>` header that says nothing about the compiler being too old. The `linux` preset pins `CMAKE_CXX_COMPILER=g++-14` so this normally never fires; it is there for anyone configuring by hand on a distro whose default `g++` is 13 (Ubuntu 24.04 / Mint 22, for instance).

Every executable lands under `build/bin/<TargetName>/Debug/`, set once in the root `CMakeLists.txt` via `set(CMAKE_RUNTIME_OUTPUT_DIRECTORY "${CMAKE_BINARY_DIR}/bin/$<TARGET_PROPERTY:NAME>/$<CONFIG>")` rather than per-target properties, so new targets pick it up automatically. You can also open the `build/` folder in Visual Studio (or generate the solution with `cmake --preset windows` and open `build\CppSandbox.sln`, named after the root `project(CppSandbox ...)` call rather than any individual sub-project) to build/run/debug individual targets from the IDE. VS reads `CMakePresets.json` natively. The root-level `CreateVisualStudioSolution.bat` is a two-line double-click wrapper (`cmake --preset windows` + `pause`) for generating the solution without typing the command by hand; it deliberately holds no flags of its own, so it cannot drift from `CMakePresets.json`.

On Linux the editor setup is VS Code with the CMake Tools extension; `.vscode/` is committed and holds only what the presets cannot: `extensions.json` (CMake Tools + cpptools), `settings.json` (`cmake.useCMakePresets`, `cmake.setBuildTargetSameAsLaunchTarget` so F5 builds only the target being debugged, and pointing IntelliSense at CMake Tools as its configuration provider so `<RAIIGarage/...>` includes resolve in the editor), `launch.json` (one gdb config that debugs whichever target the status bar has selected, via `${command:cmake.launchTargetPath}`, which also builds before launching since `cmake.buildBeforeRun` defaults to on), and `tasks.json` (the default Ctrl+Shift+B build task, building the selected launch target through the active build preset). The `cwd` in `launch.json` is `${command:cmake.launchTargetDirectory}` rather than the workspace root because LangCatalog reads `languages.txt` by relative path from next to its binary — the `VS_DEBUGGER_WORKING_DIRECTORY` property that handles this in Visual Studio is a no-op everywhere else.
...
- `RAIIGarageTests`: the six `*Tests.cpp` files in `source/RAIIGarage/tests/` (plus `CapturedOutput.h`), linked against `RAIIGarageLib` and `GTest::gtest_main`, registered with `gtest_discover_tests()`.
...
- **build-windows** and **build-linux**: configure, build all targets in Debug, `ctest`, then `RAIIGarageTests --gtest_shuffle --gtest_repeat=5`. Both drive the presets rather than spelling out flags, so CI and a local checkout cannot drift; the Linux job installs `g++-14` and `ninja-build` first, since `ubuntu-latest` still defaults to g++-13. That last step exists because CTest runs each case in its own process, which hides cross-test contamination through the `Registry` singleton no matter how cases are ordered; running the whole suite in one shared process in random order is what actually proves `Registry::Reset()` in `RegistryTest::SetUp` does its job. Keep it if you touch the test wiring.
...
- LangCatalog expects `languages.txt` to be present in the working directory. A `POST_BUILD` command in its `CMakeLists.txt` copies the file next to the **binary** (`$<TARGET_FILE_DIR:LangCatalog>`, i.e. `build/bin/LangCatalog/Debug/`), and both `VS_DEBUGGER_WORKING_DIRECTORY` and the `cwd` in `.vscode/launch.json` point the debugger at that same binary folder, so running from either IDE just works. Running it from the repo root by hand does not.
...
- **`Vehicles.h`/`.cpp`**: `IVehicle` is an abstract base with a **private** pure-virtual `DeleteVehicle()`; concrete vehicles (`Car`, `Van`, `Forklift`, `Truck`) implement it as `delete this`. Alongside the pure-virtual `Print()`, the types expose ordinary public accessors (`IVehicle::GetSpeed`, `Car::GetName`, `Van`/`Forklift::GetCargoMax`, `Forklift::GetBalance`, `Truck::GetTrailerCount`/`GetMaxLoad`) that the tests assert against instead of scraping printed output. Construction goes only through `IVehicle::Helper` (a factory/deleter class nested inside `IVehicle`; each vehicle declares `friend IVehicle`, and `Helper` reaches their private `Setup(...)` through it, since a nested class has its enclosing class's access rights), which offers three creation flavors: `CreateVehicle` (`std::unique_ptr` with `Helper` as deleter), `CreateVehicleCustom` (the project's own `UniquePtr` with `Helper` as deleter), and `CreateVehicleShared` (`std::shared_ptr`). `Helper::operator()` is what actually invokes the private `DeleteVehicle()`; this is why vehicles can't be deleted except through the smart pointers this factory hands out.
...
- **`ScopedTimer.h`**: RAII stopwatch that prints elapsed milliseconds on destruction. Used at the top of `main()`, but also held as a private member of `IVehicle`, so **every vehicle prints a timer line when it is destroyed** — that is where the timer output interleaved with vehicle output comes from.
