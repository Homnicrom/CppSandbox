...
The root `CMakeLists.txt` also `FetchContent`s GoogleTest, but as of this writing no project actually has any `TEST()`/`TEST_F()` cases: it's wired up but unused. There is no lint config. `.github/workflows/ci.yml` defines a Windows/MSVC build job, a Windows/MSVC-native AddressSanitizer job, and a Linux/g++ AddressSanitizer job, the latter two both exercising the MemorySafetyDrills drill, but it hasn't run yet since the repo isn't on GitHub.
